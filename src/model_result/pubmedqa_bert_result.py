...
def main(data_json_file, report_file="bert_result.txt"):
    logging.info(f"Loading data file: {data_json_file}")
    # 只要每行都是一个json对象，无论后缀是什么都可以
    ...
    logging.info(f"Starting evaluation on {len(data)} samples.")

    valid_indices = []
    for item in data:
        idx = item.get("idx")
        gold_support = item.get("output", "").strip()
        llm_support = item.get("llm_output", "").strip()
    ...
    with open(report_file, "w", encoding="utf-8") as fout:
        for i, idx in enumerate(valid_indices):
            ...
            fout.write(f"Gold output: {gold_sents[i]}\n")
            fout.write(f"LLM output:  {pred_sents[i]}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--answer", required=True, help="包含output和llm_output的json/jsonl文件")
