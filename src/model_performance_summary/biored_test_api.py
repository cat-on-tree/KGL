...
def main():
    ...
    parser.add_argument("--log", default="deepseek-r1-distill-qwen-1.5b-bioRED-model-test.log", help="日志文件名")
    ...
    for idx, obj in enumerate(tqdm(items, desc="Refine label by LLM")):
        ...
        system_prompt = obj["system"].strip() + '\nonly output a json contains a label, the output format examples are as follows: { "label": "Association" } or { "label": "bind" } or { "label": "Cotreatment" }'
        user_prompt = obj["llm_output"].strip()
        try:
